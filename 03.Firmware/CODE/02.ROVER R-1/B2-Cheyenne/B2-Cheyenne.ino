...
uint16_t reset_register = 0;         // Variable que almacena el valor leido del registro de la causa del reset.
uint16_t volt_to_display = 0;        // Variable temporal que almacena el valor a ser mostrado por el display cuando no se muestra el real.
...
uint16_t flag_longpress = false; // Flag que indica la presencia de un lonpress
...
//------------------------ CALCULO INICIAL CAPACIDAD ----------------------------
    ...
    else if (reset_cause == C_RCAUSE_WDT)
    {
        ...
        SaveEeprom();
#ifdef SERIAL_DEBUG
        Serial5.println("Grabado Correcta de EEPROM");
#endif
    }
...
#endif
                        flag_longpress = 2;
                        volt_to_display = theory_Vout + 10;
                        volt_to_display = constrain(volt_to_display, 50, 120);
                        DisplayVolt(volt_to_display);
